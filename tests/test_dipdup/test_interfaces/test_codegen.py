...
from dipdup.interfaces.codegen import (
    AbstractInterfacesPackageGenerator,
    InterfaceGenerator,
    InterfacesPackageGenerator,
...
class TestCodegen:
    ...
    def test_interface_render(self, interface_generator: InterfaceGenerator, expected_line: str) -> None:
        generated = interface_generator.render()
        assert expected_line in generated.splitlines()

    @pytest.mark.asyncio
    async def test_interface_generator(self, interface_generator: InterfaceGenerator, mocker: MockerFixture):
        ...
        mocked_render = mocker.patch.object(interface_generator, 'render')

        assert mocked_info.call_count == 0
        assert mocked_render.call_count == 0
    ...
    @pytest.mark.asyncio
    async def test_null_interface_package_generator(self, logger: Logger, mocker: MockerFixture):
        mocked_info = mocker.patch.object(logger, 'info')
    ...
    @pytest.mark.asyncio
    async def test_abstract_interface_package_generator(self):
